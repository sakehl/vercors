...
class Substituter(override val source: ProgramUnit, val var_substitutes: Map[String, ASTNode]) extends AbstractRewriter(source) {

  val frameStack: mutable.Stack[mutable.Set[DeclarationStatement]] = mutable.Stack()

  //val var_substitutes : Map[String, ASTNode]
  /**
  ...
  override def visit(e: NameExpression): Unit = {
    e.getKind match {
      ...
      case _ =>
        Abort("missing case %s %s in substituter", e.getKind, e.getName)
    }
    super.visit(e)
  }
  ...
  /**
    * Finds the first declaration in the frame stack that has name `name`
    */
  ...
  override def visit(assignment: AssignmentStatement): Unit = {
    ...
    val value = assignment.expression.apply(this)
    result = create assignment(assignment.location, value)
  }
  ...
  override def visit(s: ForEachLoop): Unit = {
    ...
    // TODO: Check if something needs to be done here, unsure
  ...
  override def visit(b: BlockStatement): Unit = {
    // Declarations in init blocks are supposed to leak into other components of LoopStatement.
  ...
  override def visit(e: OperatorExpression): Unit = if ((e.operator eq StandardOperator.StructDeref) || (e.operator eq StandardOperator.StructSelect)) {
    //TODO
    Abort("Unsure what to do for this operator expression '%s', somehow name scanner only looks at first argument here?", e)
    e.first.accept(this)
  }
  else super.visit(e)
  ...
  override def visit(invokation: MethodInvokation): Unit = {
    ...
    // TODO: Check if something needs to be done here, unsure. AbstractRewriter does not have this variable
    //auto_before_after = false
  }
