...
public class ExpressionTransformer<T> {
    ...
    private Assign<T> decode_assignment(Expr<T> left, String op, Expr<T> right) {
        Expr<T> assign_value = switch (op) {
            ...
            case "^=" -> new BitXor<>(left, right, 0, new GeneratedBlame<>(), OriGen.create());
            case ">>=" -> new BitShr<>(left, right, 0, new GeneratedBlame<>(), OriGen.create());
            case "<<=" -> new BitShl<>(left, right, 0, new GeneratedBlame<>(), OriGen.create());
    ...
    private Expr<T> transform_unary_expression(UnaryExpression expr, SCClassInstance sc_inst, Expr<T> obj) {
        ...
        return switch (expr.getOperator()) {
            ...
            case "~" -> new BitNot<>(original, 0, new GeneratedBlame<>(), OriGen.create());
            case "+" -> original;
